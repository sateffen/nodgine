...
describe('Requiring the nodgine should deliver a working object', function () {
    ...
    it('should be requireable as node module', function () {
        function doRequire() {
            ...
            expect(rNodgine).to.eql(nodgine);
        }

        expect(doRequire).not.to.throwError(doRequire);
    });
    ...
    it('should globalize the four main objects on call', function () {
        ...
        expect(global.$APPLICATION).to.be(nodgine.$APPLICATION);
        expect(global.$LOGGER).to.be(nodgine.$LOGGER);
        expect(global.$ROUTER).to.be(nodgine.$ROUTER);
        expect(global.$SERVICE).to.be(nodgine.$SERVICE);
    });
    ...
    it('should throw an error when trying to load a file that does not exist', function () {
        ...
        expect(load).to.throwError();
    });

    it('should throw an error when loading a not valid json file', function () {
        function load() {
                nodgine.loadFromFile(path.resolve(__dirname, './loadfromfile/testservice.js'));
            }
    ...
    it('should throw an error when passing the wrong parameter type to loadFromFile', function () {
        ...
        data.forEach(function (value) {
            expect(load.bind(null, value)).to.throwError();
        });
    });
    ...
    describe('Loading the testnodgine.json should result in a working server', function () {
        ...
        it('should not throw an error loading the file', function () {
            function load() {
                nodgine.loadFromFile(path.resolve(__dirname, './loadfromfile/testnodgine.json'));
            }

            expect(load).not.to.throwError();
        });
        ...
        it('should have set up a reachable http server', function () {
            ...
            http.get(options, function(res) {
                ...
                res.on('end', function() {
                    ...
                    expect(data).to.be('presuccesspost');
                });
            });
        });
